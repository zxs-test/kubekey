...
if [ ${WITH_WEB_INSTALLER} = true ]; then
  ...
  if [ -f ./config.yaml ]; then
    ./kk artifact export -c config.yaml --workdir prepare -a artifact.tgz
    mkdir -p offline
    tar -xzf artifact.tgz -C offline/
    tar -xzf web-installer.tgz -C offline/
    cp config.yaml offline/
    cp kk offline/
    tar -czvf offline.tgz offline
  fi
fi
